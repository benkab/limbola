       			...
       			<div class="panel panel-default">
       				<div class="panel-body" id="view-post">
						...
						<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
							<h3>{{ $post->legende->firstname}}&nbsp;{{ $post->legende->lastname}}</h3>
							<p class="text-left">{{ $post->legende->title}}</p>
							<p class="lign"></p>
							<ul id="links">
								<li>
									<a href="{{ route('view_legendes_page') }}" data-toggle="tooltip" data-placement="top" title="Voir les legendes"><span class="icon ion-ios-undo"></span></a>
								</li>
								@if($post->legende->status == 0)
									@if((Auth::user()->email === $post->user->email) || Auth::user()->isAdmin())
										<li>
											<a href="{{ route('edit_legende', ['id' => $post->id]) }}" data-toggle="tooltip" data-placement="top" title="Editer"><span class="icon ion-edit"></span></a>
										</li>
									@endif
								@endif
							...
							<ul id="links">
								...
								<li>
									@if(Auth::user()->isAdmin())
										<li>
											<a href="{{ route('publier_Legende', ['id' => $post->id] )}}" type="button" class="btn btn-sm btn-success" id="publier">
												<span>Publier</span>
											</a>
										</li>
									@endif
								...
								@else
									@if(Auth::user()->isAdmin())
										<li>
											<a href="{{ route('retirer_Legende', ['id' => $post->id] )}}" type="button" class="btn btn-sm btn-danger" id="publier">
												<span>Retirer</span>
											</a>
										</li>
									@endif
						...
						<div class="col-lg-8 col-md-8 col-sm-12 col-xs-12 fr-view">
							<h5 id="post-body">{!! $post->legende->body !!}</h5>
						</div>
						<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
