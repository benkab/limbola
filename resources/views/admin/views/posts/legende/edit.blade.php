       		...
       		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
       			<div class="panel panel-default">
       				<div class="panel-body" id="posts">
						...
						<div class="row">
							<div class="col-xs-12">
								<form class="form-vertical" action="{{ route('post_edit_legende', ['id' => $post->id]) }}" method="Post" role="form" enctype="multipart/form-data">
									...
									<div class="form-group{{ $errors->has('firstname') ? ' has-error' : '' }}">
										<input type="text" name="firstname" id="firstname" class="form-control" placeholder="Pr&eacute;nom de la l&eacute;gende" value="{{ Request::old('firstname') ?: $post->legende->firstname }}">
										@if ($errors->has('firstname'))
								        <span class="help-block">{{ $errors->first('firstname') }}</span>
								        @endif
									</div>
									<div class="form-group{{ $errors->has('lastname') ? ' has-error' : '' }}">
										<input type="text" name="lastname" id="lastname" class="form-control" placeholder="Nom de la l&eacute;gende" value="{{ Request::old('lastname') ?: $post->legende->lastname }}">
										@if ($errors->has('lastname'))
								        <span class="help-block">{{ $errors->first('lastname') }}</span>
								        @endif
									</div>
									<div class="form-group{{ $errors->has('title') ? ' has-error' : '' }}">
										<textarea name="title" id="" rows="3" class="form-control" placeholder="Nouveau titre de l'article">{{ Request::old('title') ?: $post->legende->title}}</textarea>
										@if ($errors->has('title'))
									...
									<div class="form-group{{ $errors->has('tags') ? ' has-error' : '' }}">
								  	...
								  	<div class="form-group{{ $errors->has('body') ? ' has-error' : '' }}">
								  		<textarea id="editable" name="body" rows="15" class="form-control">{{ Request::old('body') ?: $post->legende->body}}</textarea>
								  		@if ($errors->has('body'))
