       		...
       		<div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">
       			<div class="panel panel-default">
       				<div class="panel-body">
						<h5>
							Editer la t&acirc;che
							<span class="icon ion-android-calendar pull-right"></span>
						</h5>
						<p class="lign"></p>
						<form class="form-vertical" action="{{ route('post_edited_task', ['id' => $task->id]) }}" method="Post" role="form">
							<div class="form-group{{ $errors->has('description') ? ' has-error' : '' }}">
								<textarea rows="3" name="description" id="desc" class="form-control" placeholder="T&acirc;che">{{ Request::old('description') ?: $task->description}}</textarea>
								@if ($errors->has('description'))
						        <span class="help-block">{{ $errors->first('description') }}</span>
						        @endif
							</div>
							<div class="form-group{{ $errors->has('date') ? ' has-error' : '' }}">
								<input type="date" name="date" id="date" class="form-control" placeholder="Date" value="{{ Request::old('date') ?: $task->date }}">
								@if ($errors->has('date'))
						        <span class="help-block">{{ $errors->first('date') }}</span>
						        @endif
							</div>
							<div class="form-group{{ $errors->has('time') ? ' has-error' : '' }}">
								<input type="time" name="time" id="time" class="form-control" placeholder="Heure" value="{{ Request::old('time') ?: $task->time }}">
								@if ($errors->has('time'))
						        <span class="help-block">{{ $errors->first('time') }}</span>
						        @endif
							</div>
							@if($task->type == 1)
							<p class="label label-danger">Urgent</p>
							<div class="checkbox">
							    <label>
							      <input type="checkbox" name="secondaire" value="true"> Secondaire
							    </label>
							</div>
							@else
							<p class="label label-default">Secondaire</p>
							<div class="checkbox">
							    <label>
